  ...
  return (
    <main className="addArtistPage">
      ...
      <section className="addArtistForm" aria-label="アーティスト追加フォーム">
        ...
        <button
          type="button"
          className={`imagePicker ${image ? "hasImage" : ""}`}
        ...
        <label className="formLabel" htmlFor="artist-name">
          アーティスト名
        </label>
        <input
          id="artist-name"
        ...
        <div className="profileImageSection">
          <p className="formLabel">
            プロフィール画像 <span>（任意）</span>
          </p>
          <div className="profileChoices">
            {profileChoices.map((choice) => (
              <button
                className={`profileChoice ${image === choice ? "selected" : ""}`}
                type="button"
                key={choice}
                onClick={() => setImage(choice)}
                aria-label="この画像を選択"
              >
                <img src={choice} alt="" />
              </button>
        ...
        <button type="button" className="addArtistSubmit" onClick={handleSave}>
          追加する
        </button>
      </section>
    </main>
